...
for epoch in range(num_epoches):
    ...
    for i,data in enumerate(train_loader,1):
        ...
        index=np.where(out.cpu().argmax(1)==label.cpu())
        correct=Variable(img[index],requires_grad=True).cuda()
        punishout=cnn(correct)
        punish=torch.mean(punishout)
        punish.backward()
        grad=correct.grad
        #print(grad)
        #time.sleep(1000)
        punish=torch.norm(grad,p=2)
        #print(punish)
        loss = criterion(out,label)+punish*PUNISH
